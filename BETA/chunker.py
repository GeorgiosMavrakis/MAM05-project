...
Input: combined.json (your filtered diabetic medication data)
Output: chunks.jsonl (ready for embedding)
"""
...
# Your manually created drug family mapping
# Key: generic drug name (lowercase)
# Value: drug family/class
DRUG_FAMILY_MAPPING = {
...
# Input/Output paths
INPUT_FILE = "combined.json"  # Your filtered data
...
def extract_generic_names_and_class(drug: Dict[str, Any]) -> tuple[List[str], Optional[str]]:
    """
    Extract generic names and drug class using your manual mapping.

    Strategy:
    1. Try openfda generic_name first
    2. If not found, search spl_product_data_elements against DRUG_FAMILY_MAPPING
    3. Return matched generic names and corresponding drug class
    ...
    # Fallback: Search spl_product_data_elements against your mapping
    if not generic_names and "spl_product_data_elements" in drug:
        spl_text = " ".join(drug["spl_product_data_elements"]).lower()

        for drug_name, family in DRUG_FAMILY_MAPPING.items():
            if drug_name.lower() in spl_text:
                generic_names.append(drug_name)
                if not drug_class:
                    drug_class = family
...
def create_searchable_variations(brand_name: str, generic_names: List[str]) -> List[str]:
    ...
    NOTE: This is optional. The LLM could handle spelling variations,
    but this provides a safety net for:
    - Case insensitivity (Ozempic vs ozempic)
    - Spacing variations (Sodium Fluoride vs SodiumFluoride)
    - Common concatenations

    REMOVE LATER IF: LLM handles all variations perfectly.
    """
    variations = set()
...
def split_into_sentences(text: str) -> List[str]:
    ...
    # This is simple but works for most FDA text
...
def create_chunks_for_drug(drug: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    # NOTE: This is optional - can be removed if LLM handles variations well
    # Provides safety net for case/spacing differences
    searchable_variations = create_searchable_variations(brand_name, generic_names)

    # Process all fields (per your note: all fields are important)
    ...
    for field_name, field_value in drug.items():
        ...
        if isinstance(field_value, list):
            ...
            # Skip non-text fields (shouldn't happen in FDA data)
            continue
        ...
        for idx, chunk_text in enumerate(field_chunks):
            chunk = {
                ...
                "metadata": {
                    ...
                    # OPTIONAL: Searchable variations for fuzzy matching
                    # Remove this field if LLM handles all variations perfectly
...
def create_chunks(input_file: str = INPUT_FILE, output_file: str = OUTPUT_FILE) -> int:
    ...
    Args:
        input_file: Path to combined.json
