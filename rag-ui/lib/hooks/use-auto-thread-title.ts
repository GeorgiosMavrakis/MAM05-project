  ...
  useEffect(() => {
    ...
    const trySetTitleForThread = (threadId: string) => {
      try {
        ...
        // Silently ignore errors for deleted threads
        console.log("[AutoThreadTitle] Thread no longer exists (likely deleted):", threadId);
        // Clean up the processed threads ref
        processedThreadsRef.current.delete(threadId);
        delete lastMessageCountRef.current[threadId];
      }
    };
